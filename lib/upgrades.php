...
if (!elgg_get_plugin_setting('migrate_wire_to_wall', 'hypeWall')) {
	...
	$posts = new ElggBatch('elgg_get_entities_from_metadata', [
		'types' => 'object',
		'subtypes' => 'thewire',
		'metadata_names' => ['origin', 'method'],
		'metadata_values' => 'wall',
		'limit' => 0,
	]);

	$posts->setIncrementOffset(false);

	$subtype = Post::SUBTYPE;
	$subtype_id = get_subtype_id('object', Post::SUBTYPE);
	$dbprefix = elgg_get_config('dbprefix');
	...
	foreach ($posts as $post) {
		...
		update_data("
			...
			WHERE guid = $post->guid
		");

		update_data("
			UPDATE {$dbprefix}river
			SET view = 'river/object/hjwall/create', subtype = '$subtype'
			WHERE object_guid = $post->guid AND action_type = 'create'
		");

		update_data("
			UPDATE {$dbprefix}system_log
...
if (!elgg_get_plugin_setting('reverse_attachments', 'hypeWall')) {
	...
	$posts = new ElggBatch('elgg_get_entities', [
		...
		'limit' => 0,
		'joins' => [
			"JOIN {$dbprefix}entity_relationships er ON er.guid_two = e.guid AND er.relationship = 'attached'",
		],
		'group_by' => 'er.guid_two',
		'callback' => false,
	]);
